...
CREATE TABLE IF NOT EXISTS app_public.test_table (
  user_id uuid PRIMARY KEY DEFAULT public.uuid_generate_v1mc (),
  data jsonb
);

GRANT USAGE ON SCHEMA app_public TO app_postgraphile, app_anonymous, app_person;

ALTER TABLE app_public.test_table disable ROW level SECURITY;

-- Anyone can read data from this table
GRANT SELECT ON TABLE app_public.test_table TO app_postgraphile, app_anonymous, app_person;

-- Only logged in people can update & delete it.
-- This RBAC helps us differentiate guest from users

GRANT INSERT, UPDATE, DELETE ON TABLE app_public.test_table TO app_postgraphile, app_person, app_anonymous;
